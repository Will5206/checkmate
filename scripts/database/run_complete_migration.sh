...
# Script to run the complete column migration
# This adds a boolean 'complete' column to the receipts table
...
echo -e "${YELLOW}Running migration: Add 'complete' column to receipts table${NC}"
echo ""
...
read -p "This will add a 'complete' column to the receipts table. Continue? (y/n) " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
...
if [ $? -eq 0 ]; then
    ...
    echo ""
    echo "The 'complete' column has been added to the receipts table."
    echo "Existing receipts have been updated based on their item claim status."
    echo ""
    echo -e "${YELLOW}Next steps:${NC}"
    echo "1. Restart your backend server"
    echo "2. Test that receipts appear in the correct sections (Pending vs History)"
else
    echo ""
